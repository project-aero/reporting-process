...
# Visual Variables
## Colors
color.7day <- unipalette['blue']
color.7day.imperfect <- unipalette['pink']
color.30day.imperfect <- unipalette['red']
color.I <- rgb(0,0,0,.35)
color.crit <- unipalette['green']
color.ac.I <- unipalette['black']
color.ac.I.lag1 <- unipalette['black']
color.ac.I.lag7 <-  unipalette['blue']
color.ac.I.lag30 <- unipalette['red']
color.ac.snap7 <- unipalette['lightblue'] 
color.ac.snap30 <- unipalette['pink']
...
margins = c(4,5,1,9.5)+0.1
top.pannel <- c(0,1,.45,1) # panel bounds: x0,x1,y0,y1 as fraction of figure region
bottom.pannel <- c(0,1,0,.55) # panel bounds: x0,x1,y0,y1 as fraction of figure region
...
## I, lag 1
lines(subset(sim.7di.I,interval==1 & bandwidth==35 & lag==1)$stats[[1]]$autocorrelation,
      col=color.ac.I.lag1, lwd=lwd.I)

## weekly snapshots of I, lag 1
lines(subset(sim.7di.I, interval==7 & bandwidth==35 & lag==1)$stats[[1]]$autocorrelation,
      col=color.ac.snap7, lwd=lwd.I)
...
legend("topleft", xpd=NA, inset=c(1.01,0), xjust=0, yjust=0, cex=.65, y.intersp = 3,
       legend=c(
         TeX(sprintf("Number Infected, $\\tau = $ %.2f", 
                     subset(sim.7di.I, interval==1 & bandwidth==35 & lag==1)$taus[[1]]$autocorrelation)),
         TeX(sprintf("Weekly Snapshots\nof Number Infected, $\\tau = $ %.2f", 
                     subset(sim.7di.I, interval==7 & bandwidth==35 & lag==1)$taus[[1]]$autocorrelation)),
         TeX(sprintf("Weekly Reports, $\\tau = $ %.2f", 
                     subset(reports.7di, tau==7 & bandwidth==35 & rho==2^-3.2 & disp==100)$taus[[1]]$autocorrelation)),
         TeX(sprintf("Monthly Reports, $\\tau = $ %.2f", 
       ...
       lwd=c(lwd.I,lwd.I,lwd.7day,lwd.30day),
       lty=1,
       bty='n')
...
lines(subset(reports.7di, tau==7 & bandwidth==35 & rho==2^-3.2 & disp==100)$data[[1]], 
      type='s', pch=20, lwd=lwd.7day, col=color.7day.imperfect)
...
legend("topleft", xpd=NA, inset=c(1.01,0), xjust=0, yjust=0, cex=.65, y.intersp = 1.5,
       ...
       col=c(color.I,color.7day.imperfect, color.30day.imperfect),
       lty=1,
       lwd=c(9,lwd.7day,lwd.30day),
       bty='n')
...
# Data
  ...
  reports.7di.7dr.perfect.autocorrelation <- subset(reports.7di, tau==7 & bandwidth==35 & rho==1 & is.na(disp))$autocorrelation
  reports.7di.7dr.imperfect.autocorrelation <- subset(reports.7di, tau==7 & bandwidth==35 & rho== .25 & disp==0.1)$autocorrelation

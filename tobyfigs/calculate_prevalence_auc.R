...
get_auc <- function(df){
  ...
  new_levels <-  c("Mean", "Variance", "Var. 1st Diff", 
                   ...
                   "Kurtosis")
  out <- out[c(6,1,2,7,3,4,5,8,9,10),]
...
#Simulates 100 replicates, N=1e6, 20yrs of data, rng seed = 1. 
#Takes ~ 10 minutes on a standard macbook

calculate_auc <- function(infectious_period){
  
  sir_command <- paste("./sir_sim 10 1000000 20 1",c("-e","-n"),infectious_period)
  emerge <- read.table(text = system(sir_command[1], intern = TRUE))
  not_emerge <- read.table(text = system(sir_command[2], intern = TRUE))
  emerge_ews <- get_ews(emerge)
  not_ews <- get_ews(not_emerge)
